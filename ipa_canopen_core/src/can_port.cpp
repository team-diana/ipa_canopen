...
void CanPort::sendMsg(int canId, const std::vector<BYTE>& data, int rtr)
{
    CAN_PACKET canPacket;
    memset(&canPacket, 0, sizeof(canPacket));
    canPacket.CAN_ID = canId;
    canPacket.rtr = rtr;
    canPacket.len = data.size(); /// WARNING: we assume that each element in `data' is a byte

    for (int i = 0; i < data.size(); ++i)
    {
      canPacket.data[i] = data[i];
    }

    sendCanPacket(canPacket);
  }

CAN_PACKET CanPort::rcvMsg() {
